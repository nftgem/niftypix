...
casper.test.begin("Settings Test", 18, function (test) {
  ...
  function onTestStart() {
    test.assertExists(
      ...
      "Piskel ready, test starting"
    );

    test.assert(!isDrawerExpanded(), "settings drawer is closed");
    test.assertDoesntExist(
      ".settings-section-resize",
      "Check if resize settings drawer is closed"
    );
  ...
  function onResizePanelReady() {
    ...
    test.assertExists(
      ".settings-section-resize",
      "Check if resize panel is opened"
    );

    // Open preferences settings panel.
    test.assertDoesntExist(
      ".settings-section-preferences",
      "Check if preferences settings panel is closed"
    );
    this.click('[data-setting="user"]');

    this.waitForSelector(
      ".settings-section-preferences",
      onPreferencesPanelReady,
      test.timeout,
      10000
    );
  }
  ...
  function onPreferencesPanelReady() {
    ...
    test.assertExists(
      ".settings-section-preferences",
      "Check if preferences settings panel is opened"
    );

    // Open save panel.
    test.assertDoesntExist(
      ".settings-section-save",
      "Check if save panel is closed"
    );
  ...
  function onSavePanelReady() {
    ...
    test.assertExists(
      ".settings-section-save",
      "Check if save panel is opened"
    );

    // Open export panel.
    test.assertDoesntExist(
      ".settings-section-export",
      "Check if export panel is closed"
    );
  ...
  function onExportPanelReady() {
    ...
    test.assertExists(
      ".settings-section-export",
      "Check if export panel is opened"
    );

    // Open import panel.
    test.assertDoesntExist(
      ".settings-section-import",
      "Check if import panel is closed"
    );
  ...
  function onImportPanelReady() {
    ...
    test.assertExists(
      ".settings-section-import",
      "Check if import panel is opened"
    );

    // Click on import again to close the settings drawer.
    this.click('[data-setting="import"]');
    this.waitForSelector(
      '[data-pskl-controller="settings"]:not(.expanded)',
      onDrawerClosed,
      test.timeout,
      10000
    );
